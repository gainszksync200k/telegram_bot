...
# Ваш API токен
TOKEN = "YOUR_BOT_API" # УКАЖИТЕ СЮДА АПИ БОТА. ПОЛУЧЕНЫЙ В ШАГЕ 2.

# Ваш собственный ID пользователя
SELF_CHAT_ID = "YOUR_ID_TELEGRAM" # УКАЖИТЕ СЮДА ID ТЕЛЕГИ. ПОЛУЧЕНЫЙ В ШАГЕ 3.

# ЧАСТОТА ОТПРАВЛЕНИЙ СООБЩЕНИЯ, В МИНУТАХ
TIME = 15 # Я СДЕЛАЛ 15 МИНУТ, МОЖНО РЕЖЕ, НО НЕ МЕНЬШЕ 5 МИНУТ, А ТО ВАШ АПИ ЗАБАНЯТ ЗА ЧАСТЫЕ ЗАПРОСЫ!!!!
